...
def display_banner():
    ...
    print(Fore.RED + pyfiglet.figlet_format("N-OPPENHIMER"))
    print(Fore.GREEN + "Made by @oppenhimer\n")
...
async def process_round(round_num, sessions_info):
    ...
    # Start new client for each session.
    ...
    # Disconnect all clients to kill sessions and free resources
    disconnect_tasks = []
    for session_name, client in clients.items():
        disconnect_tasks.append(client.disconnect())
    await asyncio.gather(*disconnect_tasks)

    # Clear memory for each round
    clients.clear()
    gc.collect()
...
async def main():
    """Main function to handle auto forwarding rounds.
